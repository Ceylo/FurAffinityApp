...
shared caller compiles unchanged. It lives in the app module because it is a
*bridged* view — skipstone generates the Kotlin glue its `@State` needs, and it only
processes this module — and because it needs skip-web, which FAKit does not depend
on. It is unguarded, so the Darwin bridge compile finds it too; this module's
declaration shadows FAKit's.
...
- **The stage flags are mirrored into the view's own `@State`.** Reading
  ...
  app-module cases, **this one cannot be fixed by an import alone.** Adding
  ...
  module 'CJNI'`. Note the mechanism, because it was long recorded wrongly here:
  `CJNI` is **not** generated by skipstone. It is an ordinary SwiftPM C target in
  `swift-jni` (`Sources/CJNI/{cjni.c,include/module.modulemap}`), and SwiftPM puts
  its modulemap on the Clang search path of any target whose *dependency closure
  reaches that C target*. So the failure is a missing or incomplete dependency
  edge, not a plugin that refuses to run. The mirror stays until FAKit is set up
  with those edges.
