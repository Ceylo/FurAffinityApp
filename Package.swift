...
let package = Package(
    ...
    dependencies: [
        ...
        // Forked for `listRowInsets` and `Text(AttributedString)` (both unavailable
        // upstream in both repos) — see Android/README.md §Forks. skip-ui comes in
        // transitively via skip-fuse-ui, so it must be overridden here too.
